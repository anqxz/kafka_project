...
**Status:** PR 1 in flight on branch `feat/pr1-network-segmentation` — runbook + verification in [doc 01 §6](01-NETWORK-ARCHITECTURE.md#6-pr-1--implementation-runbook).
...
2. **PR 2 — Healthchecks + pinned images:** doc 02 §4 + doc 04 F8.
3. **PR 3 — Observability v2 (metrics + alerting):** scrape jobs per system, **kminion**, **blackbox-exporter**, **Alertmanager + ntfy** (Watchdog deadman), rule files from doc 05.
4. **PR 4 — Observability v3 (LGTM+P):** **otel-collector** (single pipeline, replaces per-signal shippers), **Loki** (logs), **Tempo** + OTel instrumentation (traces), **Pyroscope** on the Connect JVM (profiles), Grafana correlation.
5. **PR 5 — Cruise Control:** metrics-reporter JAR in brokers, CC service, goals config, anomaly detection (self-healing off) — doc 02 §3.5.
